...
import { MessageSquare, Copy, Check, Star, HelpCircle, Heart } from 'lucide-react';
...
export function PropertyCommentsModal({ property, open, onOpenChange }: PropertyCommentsModalProps) {
  ...
  const handleCopyQuestions = () => {
    ...
    const text = `Olá! Tudo bem? Estou analisando o imóvel "${property.titulo}" (${property.bairro}) e gostaria de tirar as seguintes dúvidas:\n\n${property.duvidasCorretor}`;
    ...
    <Dialog open={open} onOpenChange={onOpenChange} maxWidth="lg">
      <div className="p-4 bg-white dark:bg-slate-900 rounded-2xl space-y-4">
        <DialogHeader>
          <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
            <div className="flex items-center gap-2">
              <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-indigo-100 dark:bg-indigo-950 text-indigo-600 dark:text-indigo-300 font-bold">
              ...
              <div>
                ...
                <DialogDescription className="text-xs text-slate-500">
                  {property.bairro} — Opiniões do Casal & Pergunta para o Corretor
                </DialogDescription>
              </div>
            </div>
            <span className="text-xs font-bold px-2.5 py-1 rounded-full bg-rose-50 text-rose-600 dark:bg-rose-950 dark:text-rose-300 border border-rose-200 dark:border-rose-800">
        ...
        <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-1">
          {/* Saymon Opinion */}
          <div className="rounded-2xl border border-indigo-200 dark:border-indigo-900/60 bg-indigo-50/40 dark:bg-indigo-950/30 p-4 space-y-2">
            ...
            <p className="text-xs text-slate-700 dark:text-slate-200 font-normal leading-relaxed bg-white/80 dark:bg-slate-900/80 p-3 rounded-xl border border-indigo-100 dark:border-indigo-900/40 italic">
              {property.opiniaoSaymon || 'Nenhum comentário cadastrado pelo Saymon.'}
            </p>
          </div>

          {/* Kelly Opinion */}
          <div className="rounded-2xl border border-rose-200 dark:border-rose-900/60 bg-rose-50/40 dark:bg-rose-950/30 p-4 space-y-2">
            <div className="flex items-center justify-between">
            ...
            <p className="text-xs text-slate-700 dark:text-slate-200 font-normal leading-relaxed bg-white/80 dark:bg-slate-900/80 p-3 rounded-xl border border-rose-100 dark:border-rose-900/40 italic">
              {property.opiniaoKelly || 'Nenhum comentário cadastrado pela Kelly.'}
            </p>
          </div>

          {/* Questions for Realtor (High Contrast Box) */}
          <div className="rounded-2xl border border-amber-200 dark:border-amber-900/60 bg-amber-50/40 dark:bg-amber-950/30 p-4 space-y-3">
            <div className="flex items-center justify-between">
            ...
            <div className="bg-white dark:bg-slate-900 p-3 rounded-xl border border-amber-200/80 dark:border-amber-900/40 text-xs text-slate-800 dark:text-slate-100 whitespace-pre-wrap font-medium leading-relaxed">
